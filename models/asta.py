...
class Attention(nn.Module):
	def __init__(self, network_depth, dim, num_heads, window_size, shift_size, use_attn=False, conv_type=None):
		...
		self.adapter = nn.Sequential(
			nn.Conv2d(72, 72, kernel_size=3, padding=1, bias=False, padding_mode='reflect'),
			nn.ReLU(inplace=True),
		)
		self.conbination = nn.Linear(128, 64)
		...
		if self.use_attn:
			...
			self.attn2 = WindowAttention(dim, window_size - 4, num_heads)
		self.apply(self._init_weights)
	...
	def forward(self, X):
		...
		if self.use_attn:
			...
			if shifted_QKV.size(1) == 72:
				shifted_QKV = self.adapter(shifted_QKV)
			shifted_QKV = shifted_QKV.permute(0, 2, 3, 1)
			qkv1 = window_partition(shifted_QKV, self.window_size)  # nW*B, window_size**2, C
			qkv2 = window_partition(shifted_QKV, self.window_size - 4)
			
			attn_windows1 = self.attn1(qkv1)
			attn_windows2 = self.attn2(qkv2)
			attn_windows2 = attn_windows2.reshape(attn_windows2.shape[0] // 4, 16 * 4, attn_windows2.shape[-1])
			attn_windows = torch.cat((attn_windows1, attn_windows2), dim=1)

			attn_windows = attn_windows.permute(0, 2, 1)
			attn_windows = self.conbination(attn_windows)
			attn_windows = attn_windows.permute(0, 2, 1)
